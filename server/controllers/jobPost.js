...
export const getJobPostById = async (req, res) => {
  try {
    ...
    const jobPost = await getOrSetCache(
      cacheKey,
      async () => {
...
export const getAllJobPosts = async (req, res) => {
  try {
    ...
    const jobPosts = await getOrSetCache(
      key,
      async () => {
        return await JobPost.find({ recruiter: recruiterId })
          .sort({ createdAt: -1 })
          .lean();
      },
      60
...
export const updateJobPost = async (req, res) => {
  try {
    ...
    const updatedJobPost = await JobPost.findOneAndUpdate(
      { _id: jobPostId, recruiter: recruiterId },
      filteredData,
      { new: true }
    );
    if (!updatedJobPost) {
...
export const getApplicationsForJobPost = async (req, res) => {
  try {
    ...
    const applications = await getOrSetCache(
      cacheKey,
      async () => {
  ...
  try {
    ...
    if (since) {
      ...
      if (isNaN(sinceDate)) {
        return res
          .status(400)
          .json({ message: "Invalid 'since' date format." });
      }
...
export const getApplicationDetails = async (req, res) => {
  try {
    ...
    if (!jobPost) {
      return res
        .status(403)
        .json({ message: "Unauthorized or job post not found" });
    }
...
export const updateStatus = async (req, res) => {
  try {
    ...
    const job = await Job.findOne({ jobPostId, userId }).populate(
      "jobPostId",
      "recruiter"
    );
    ...
    return res
      .status(200)
      .json({ message: "Job status updated successfully", job });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "An error occurred", error: error.message });
...
export const deleteJobPost = async (req, res) => {
  try {
    ...
    if (!jobPost) {
      return res
        .status(404)
        .json("Job post not found or you do not have the authorization");
    }
