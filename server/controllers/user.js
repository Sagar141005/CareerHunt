...
export const socialLogin = async (req, res) => {
  try {
    ...
    if (!passportUser || !passportUser.emails || !passportUser.emails.length) {
      return res
        .status(400)
        .json({ message: "No email received from provider" });
    }

    const email = passportUser.emails[0].value;
    const name =
      passportUser.displayName ||
      `${passportUser.name?.givenName || ""} ${
        passportUser.name?.familyName || ""
      }`.trim() ||
      "User";
    ...
    return res
      .status(500)
      .json({ message: "Social auth error", error: error.message });
  }
};
  ...
  try {
    ...
    if (company) {
      ...
      if (company.logoUrl !== undefined)
        updatedFields.company.logoUrl = company.logoUrl;
      if (company.website !== undefined)
        updatedFields.company.website = company.website;
      if (company.location !== undefined)
        updatedFields.company.location = company.location;
    }

    if (
      updatedFields.company &&
      Object.keys(updatedFields.company).length === 0
    ) {
