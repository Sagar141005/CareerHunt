...
export const getAvailableJobs = async (req, res) => {
  try {
    ...
    const stringify = (val) =>
      Array.isArray(val) ? val.sort().join(",") : val || "all";

    const cacheKey = `availableJobs:${stringify(title)}:${stringify(
      location
    )}:${stringify(type)}:${stringify(typeQuery)}:${stringify(
      employmentType
    )}:${stringify(level)}:${stringify(levelQuery)}:${stringify(department)}:${
      minSalary || 0
    }:${maxSalary || "max"}:${search || "none"}`;
    ...
    const jobs = await getOrSetCache(
      ...
      async () => {
        ...
        addPartialMatchFilter(
          "employmentType",
          normalizeToArray(employmentType)
        );
    ...
    const userJobsMap = new Map(
      userJobs.map((job) => [
        job.jobPostId.toString(),
        { isSaved: job.isSaved, status: job.status },
      ])
    );
...
export const getPublicJobPostById = async (req, res) => {
  try {
    ...
    const job = await Job.findOne({ userId, jobPostId: id }).populate(
      "jobPostId"
    );
...
export const applyToJob = async (req, res) => {
  try {
    ...
    if (job) {
      ...
      if (disallowedStatuses.includes(currentStatus)) {
        return res.status(400).json({
          message: `Cannot apply — already in status: "${currentStatus}"`,
        });
      }
    ...
    return res
      .status(201)
      .json({ message: "Successfully applied to job", job });
  } catch (error) {
    return res
      .status(500)
...
export const toggleSaveJob = async (req, res) => {
  try {
    ...
    if (typeof isSaved !== "boolean") {
      return res
        .status(400)
        .json({ message: "`isSaved` must be true or false" });
    }
    ...
    if (!job) {
      ...
      if (job.isSaved === isSaved) {
        return res
          .status(200)
          .json({ message: `Job already ${isSaved ? "saved" : "unsaved"}` });
      }
    ...
    return res
      .status(200)
      .json({ message: isSaved ? "Job saved" : "Job unsaved" });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Error saving/unsaving job", error: error.message });
  }
};
...
export const withdrawApplication = async (req, res) => {
  try {
    ...
    if (disallowedStatuses.includes(job.status)) {
      return res.status(400).json({
        message: `Cannot withdraw from job in status "${job.status}"`,
      });
    }
    ...
    return res
      .status(200)
      .json({ message: "Application withdrawn successfully", job });
  } catch (error) {
    return res.status(500).json({
      message: "Error while withdrawing from job",
      error: error.message,
    });
  }
};
